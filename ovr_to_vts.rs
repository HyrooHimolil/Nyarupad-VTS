...
// HMD tracking axes{{{
		let mut trackingstate: ovrTrackingState = ovr_GetTrackingState(session,0,0);
		...
		let mut vectorquat: Quaternion = new(quat.w, quat.x, quat.y, quat.z);
		let (orientation_s, ref orientation_v) = pose.orientation;
		let (HMDrx,HMDry,HMDrz) = cgmath::Quaternion::to_euler(vectorquat);
